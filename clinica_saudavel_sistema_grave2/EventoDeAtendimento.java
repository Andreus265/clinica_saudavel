...
public class EventoDeAtendimento extends EventOf2Entities<Consultorios,Pacientes> {
    ...
    public void eventRoutine(Consultorios consultorio, Pacientes paciente){
        ...
        if(!modelo.pacientesFilaGrave.isEmpty()){
            
            if(consultorio.isDisponivel()){
                Pacientes novo_paciente = modelo.pacientesFilaGrave.get(0);
                modelo.pacientesFilaGrave.remove(novo_paciente);
                consultorio.ocupar();
        ...
        else if(!consultorio.getFila().isEmpty()){
